...
def test_update_order_status_raises_error_if_not_found(order_tracker, mock_storage):
    ...
    with pytest.raises(ValueError, match="Order with ID 'NON_EXISTENT' not found."):
...
def test_list_all_orders_successfully(order_tracker, mock_storage):
    ...
    orders = {
        "ORD001": {"order_id": "ORD001", "item_name": "Laptop", "status": "pending"},
        "ORD002": {"order_id": "ORD002", "item_name": "Phone", "status": "shipped"}
    }
...
def test_list_orders_by_status_successfully(order_tracker, mock_storage):
    ...
    mock_storage.get_all_orders.return_value = {
        "ORD001": {"order_id": "ORD001", "item_name": "Laptop", "status": "pending"},
        "ORD002": {"order_id": "ORD002", "item_name": "Phone", "status": "shipped"},
        "ORD003": {"order_id": "ORD003", "item_name": "Tablet", "status": "pending"}
    }

    result = order_tracker.list_orders_by_status("pending")
    assert len(result) == 2
    assert all(order["status"] == "pending" for order in result.values())


def test_list_orders_by_status_returns_empty_if_none_found(order_tracker, mock_storage):
    """Tests that filtering by non-existent status returns empty dict."""
    mock_storage.get_all_orders.return_value = {
        "ORD001": {"order_id": "ORD001", "item_name": "Laptop", "status": "pending"},
        "ORD002": {"order_id": "ORD002", "item_name": "Phone", "status": "shipped"}
    }
